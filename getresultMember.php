...
$sql = "SELECT `name`, `phone`, `address`, `email`, `stime`, `user`.`id` from `user` LEFT JOIN  `group_user` ON `group_user`.`uid`=`user`.`id` where `group_user`.`groupId`='".$id."' " ;

$paginationlink = "getresultMember.php?page=";	
    ...
    {foreach($faq as $k=>$v) { 
      ...
      <tr>
       ...
       <td><?php echo $faq[$k]['stime'];?></td>
        ...
        <td><a href="deleteaddmember.php?id=<?php echo base64_encode($faq[$k]['id']);?>&groupId=<?php echo base64_encode($id);?>" class="btn btn-social-icon btn-google" onClick="return checkDelete()" ><i class="fa fa fa-trash-o"></i></a>
