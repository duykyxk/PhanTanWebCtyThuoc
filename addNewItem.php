...
if ($dem==12) $etime = date('Y/m/d H:i:s');
else
	$etime = null;
...
function resizeImage($file, $id, $filename){
	...
	switch ($imageType) {
		...
		imagepng($targetLayer, $fileNewName);
		break;
		...
		$targetLayer = imageResize($imageResourceId,$sourceProperties[0],$sourceProperties[1]);
		imagegif($targetLayer, $fileNewName);
		break;
		...
		$targetLayer = imageResize($imageResourceId,$sourceProperties[0],$sourceProperties[1]);
		imagejpeg($targetLayer,$fileNewName);
		break;
	...
	move_uploaded_file($file,$fileNewName);
